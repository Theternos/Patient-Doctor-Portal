...
cursor = connection.cursor()
no_patient = "select appoid, S.scheduletime, P.ptel, D.docname from appointment A, `schedule` S, patient P, Doctor D where D.docid = S.docid AND A.scheduleid = S.scheduleid AND room_flag = 0 AND P.pid = A.pid AND `mode` = 'Video Consultancy' AND DATE(S.scheduledate) = CURDATE()"
cursor.execute(no_patient)
rows = cursor.fetchall()
for row in rows:
    appoid = row[0]
    ...
    patient_tel = row[2]
    doc_name = row[3]
    message_time = fetched_time - timedelta(hours=2, minutes=0)
    current_time = datetime.now()
    t = current_time.strftime("%H:%M:%S")
